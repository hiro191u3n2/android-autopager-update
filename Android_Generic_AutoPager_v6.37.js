...
(() => {
  ...
  const HIDDEN = "generic-sumahodigest-home-segment-hidden-v637";
  const STYLE_ID = "generic-sumahodigest-home-segment-style-v637";
  const CATEGORY_LABELS = ["Xperia・Sony", "Pixel", "Galaxy"];
  ...
  const ensureStyle = () => {
    ...
    style.textContent =
      ...
      "max-height:0!important;margin:0!important;padding:0!important;border:0!important;overflow:hidden!important}";
    (document.head || document.documentElement).appendChild(style);
  };
  ...
  const hide = (element, reason) => {
    ...
    if (reason) element.dataset.genericSumahoDigestHomeV637 = reason;
    return true;
  };

  const exactHeading = (root, label) => {
    for (const element of root.querySelectorAll(HEADING_SELECTOR)) {
      if (!(element instanceof HTMLElement)) continue;
      if (compact(element.textContent) === compact(label)) return element;
    }
    return null;
  };

  const isMoreControl = (element) =>
    element instanceof HTMLElement &&
    /^もっと(?:見る|みる)$/.test(compact(element.textContent));

  const categoryHeadingCount = (node) => {
    let count = 0;
    for (const heading of node.querySelectorAll(HEADING_SELECTOR)) {
      if (CATEGORY_LABELS.some((label) => compact(heading.textContent) === compact(label))) {
        count += 1;
      }
    }
    return count;
  ...
  const findCategoryBlock = (heading, root) => {
    let node = heading;
    const candidates = [];
    for (let depth = 0; node && depth < 9; depth += 1, node = node.parentElement) {
      if (!(node instanceof HTMLElement)) continue;
      if (node === root || node === document.body || node === document.documentElement) break;
      ...
      const hasMore = Array.from(node.querySelectorAll(CONTROL_SELECTOR)).some(isMoreControl);
      if (links < 3 || links > 30 || !hasMore) continue;
      if (categoryHeadingCount(node) !== 1) continue;
    ...
    candidates.sort((a, b) => a.size - b.size || a.depth - b.depth);
    return candidates[0]?.node || null;
  };

  const hideMoreRow = (control, root) => {
    ...
    for (let depth = 0; node && depth < 6; depth += 1, node = node.parentElement) {
      ...
      if (text === compact(control.textContent) && links <= 2 && descendants <= 12) {
        best = node;
      } else {
        break;
      }
  ...
  const scan = (root = document) => {
    ...
    const headings = CATEGORY_LABELS.map((label) => exactHeading(root, label));
    if (headings.some((heading) => !heading)) return false;

    const blocks = headings.map((heading) => findCategoryBlock(heading, root));
    if (blocks.some((block) => !block)) return false;
    if (new Set(blocks).size !== blocks.length) return false;

    const xperiaHeading = headings[0];
    const moreControls = Array.from(root.querySelectorAll(CONTROL_SELECTOR)).filter(isMoreControl);
    const preceding = moreControls.filter((control) => {
      if (blocks.some((block) => block.contains(control))) return false;
      return Boolean(
        control.compareDocumentPosition(xperiaHeading) &
        Node.DOCUMENT_POSITION_FOLLOWING
      );
    });
    const latestMore = preceding[preceding.length - 1] || null;

    for (let i = 0; i < blocks.length; i += 1) {
      hide(blocks[i], "category:" + CATEGORY_LABELS[i]);
    }
    if (latestMore) hideMoreRow(latestMore, root);

    return true;
  };

  const scheduleDocumentScan = () => {
    window.setTimeout(() => scan(document), 0);
    window.setTimeout(() => scan(document), 250);
  };

  document.addEventListener("DOMContentLoaded", scheduleDocumentScan, { once: true });

  document.addEventListener("GenericAutoPagerLoaded", (event) => {
    const container = event?.detail?.container;
    if (container instanceof HTMLElement) {
      window.setTimeout(() => scan(container), 0);
    }
  }, { passive: true });

  if (document.readyState !== "loading") scheduleDocumentScan();
})();
