...
(()=>{"use strict";!function(){
  ...
  if(!/^\/(?:articles|expert\/articles|feature|pickup)(?:\/|$)/i.test(location.pathname))return;

  const INSTANCE_KEY="__androidGenericYahooNativeVideoV630";
  if(window[INSTANCE_KEY])return;
  window[INSTANCE_KEY]=true;

  const ROOT_SELECTOR=".yvpub-player[data-contentid],.yvpub-player[data-content-id],.yvpub-player";
  const STATE_ATTRIBUTE="data-generic-yahoo-native-video-v630";
  const API_ORIGIN="https://feapi-yvpub.yahooapis.jp";
  const API_APP_ID="dj0zaiZpPVZMTVFJR0FwZWpiMyZzPWNvbnN1bWVyc2VjcmV0Jng9YjU-";
  const HLS_MIME="application/vnd.apple.mpegurl";
  const metadataCache=new Map();

  function contentIdFor(root){
    const direct=String(root.getAttribute("data-contentid")||root.getAttribute("data-content-id")||"").trim();
    if(/^[1-9]\d{0,11}$/.test(direct))return direct;
    const frame=root.querySelector('iframe[src*="contentid="]');
    if(!frame)return "";
    try{
      const value=new URL(frame.getAttribute("src"),location.href).searchParams.get("contentid")||"";
  ...
  function allowedVideoUrl(value){
    try{
      ...
      if(url.hostname!=="gw-yvpub.yahoo.co.jp"&&!url.hostname.endsWith(".gw-yvpub.yahoo.co.jp"))return "";
      return url.href;
    }catch{return ""}
  }
  ...
  function parseMetadata(payload,expectedId){
    const results=payload&&payload.ResultSet&&payload.ResultSet.Result;
    const result=Array.isArray(results)?results[0]:results;
    if(!result||String(result.contentid)!==String(expectedId))throw new Error("invalid content metadata");
    const checks=result.Check||{};
    for(const key of ["Domestic","Device","Status","Period","Domain"]){
      if(Number(checks[key])!==0){
        const error=new Error("video unavailable: "+key);
        error.code="restricted";
        throw error;
      }
    }
    if(result.isCaution===true||result.IsCaution===true){
      const error=new Error("video requires caution confirmation");
      error.code="restricted";
      throw error;
    }

    const set=result.VideoUrlSet||{};
    const rows=Array.isArray(set.VideoUrl)?set.VideoUrl:[];
    const candidates=[];
    for(const row of rows){
      const url=allowedVideoUrl(row&&row.Url);
      if(!url)continue;
      const delivery=String(row.delivery||"").toLowerCase();
      const hls=delivery==="hls"||/\.m3u8(?:$|\?)/i.test(url);
      const progressive=/progressive|mp4/.test(delivery)||/\.mp4(?:$|\?)/i.test(url);
      if(!hls&&!progressive)continue;
      candidates.push({url,type:hls?HLS_MIME:"video/mp4",hls,bitrate:Number(row.bitrate)||0});
    }
    const hls=candidates.filter(item=>item.hls);
    const adaptive=hls.filter(item=>item.bitrate===0);
    const fixedHls=hls.filter(item=>item.bitrate!==0).sort((a,b)=>b.bitrate-a.bitrate);
    const progressive=candidates.filter(item=>!item.hls).sort((a,b)=>b.bitrate-a.bitrate);
    const asset=String(set.asset||"").toLowerCase();
    const ordered=asset==="packaged"?[...fixedHls,...adaptive,...progressive]:asset==="general"?[...adaptive,...fixedHls,...progressive]:[...adaptive,...progressive,...fixedHls];
    const seen=new Set();
    const sources=ordered.filter(item=>!seen.has(item.url)&&seen.add(item.url));
    if(!sources.length)throw new Error("trusted video source missing");
    return {
      sources,
      poster:safePosterUrl(result.Thumbnail&&result.Thumbnail.ThumbnailUrl),
      title:String(result.Title||"Yahoo!ニュース動画")
    };
  }
  ...
  async function requestWithFetch(url){
    ...
    const timer=window.setTimeout(()=>controller&&controller.abort(),10000);
    try{
      const response=await fetch(url,{credentials:"omit",cache:"no-store",signal:controller&&controller.signal});
      if(!response.ok)throw new Error("video metadata HTTP "+response.status);
      return await response.json();
    }finally{window.clearTimeout(timer)}
  }

  function requestWithGm(url){
    const request=typeof GM_xmlhttpRequest==="function"?options=>GM_xmlhttpRequest(options):typeof GM==="object"&&GM&&typeof GM.xmlHttpRequest==="function"?options=>GM.xmlHttpRequest(options):null;
    ...
    return new Promise((resolve,reject)=>{
      ...
      const finish=(error,response)=>{
        ...
        if(status<200||status>=300)return reject(new Error("video metadata HTTP "+status));
        try{
          if(response.response&&typeof response.response==="object")return resolve(response.response);
          resolve(JSON.parse(String(response.responseText||response.response||"")));
        }catch(parseError){reject(parseError)}
      };
      try{
        const returned=request({method:"GET",url,headers:{Accept:"application/json"},anonymous:true,timeout:10000,onload:response=>finish(null,response),onerror:()=>finish(new Error("GM video metadata request failed")),ontimeout:()=>finish(new Error("GM video metadata request timed out"))});
        if(returned&&typeof returned.then==="function")returned.then(response=>finish(null,response),error=>finish(error instanceof Error?error:new Error(String(error))));
      }catch(error){finish(error instanceof Error?error:new Error(String(error)))}
    });
  }

  async function requestMetadata(id){
    const url=API_ORIGIN+"/v1/content/"+encodeURIComponent(id)+"?appid="+encodeURIComponent(API_APP_ID)+"&output=json&domain=news.yahoo.co.jp&device_type=2110";
    let payload;
    try{payload=await requestWithFetch(url)}catch(fetchError){
      try{payload=await requestWithGm(url)}catch{throw fetchError}
    }
  ...
  function metadataFor(id){
    ...
    const pending=requestMetadata(id).catch(error=>{metadataCache.delete(id);throw error});
    metadataCache.set(id,pending);
    return pending;
  }

  function renderNativePlayer(root,info){
    if(root.getAttribute(STATE_ATTRIBUTE)==="ready")return;
    root.setAttribute("data-composed","1");
    ...
    root.style.setProperty("aspect-ratio","16 / 9");
    ...
    video.setAttribute("aria-label",info.title);
    video.setAttribute("data-stream-type",HLS_MIME);
    if(info.poster)video.poster=info.poster;
    video.style.cssText="position:absolute;inset:0;display:block;width:100%;height:100%;background:#000;object-fit:contain";

    let sourceIndex=0;
    const setSource=index=>{
      sourceIndex=index;
      video.src=info.sources[index].url;
      video.setAttribute("data-stream-type",info.sources[index].type);
      try{video.load()}catch{}
    };
    const showRetry=()=>{
      if(root.querySelector('[data-generic-yahoo-video-retry="1"]'))return;
      const panel=document.createElement("div");
      panel.setAttribute("data-generic-yahoo-video-retry","1");
      panel.style.cssText="position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;padding:20px;background:#000d;color:#fff;text-align:center;font:600 15px/1.5 system-ui,sans-serif";
      const message=document.createElement("div");
      message.textContent="動画を読み込めませんでした";
      const retry=document.createElement("button");
      retry.type="button";
      retry.textContent="動画を再試行";
      retry.style.cssText="padding:10px 16px;border:0;border-radius:8px;background:#1683e8;color:#fff;font:inherit";
      retry.addEventListener("click",()=>{panel.remove();setSource(0);video.play().catch(()=>{})});
      const direct=document.createElement("a");
      direct.href=info.sources[0].url;
      direct.target="_blank";
      direct.rel="noopener noreferrer";
      direct.textContent="動画を直接開く";
      direct.style.cssText="color:#8ec9ff";
      panel.append(message,retry,direct);
      root.appendChild(panel);
    };
    video.addEventListener("error",()=>{
      if(sourceIndex+1<info.sources.length)return setSource(sourceIndex+1);
      showRetry();
    });
  ...
  async function processRoot(root){
    ...
    const state=root.getAttribute(STATE_ATTRIBUTE);
    if(state==="pending"||state==="ready"||state==="restricted"||state==="failed")return;
    const id=contentIdFor(root);
    if(!id)return;
    root.setAttribute(STATE_ATTRIBUTE,"pending");
    try{
      const info=await metadataFor(id);
      if(root.isConnected!==false)renderNativePlayer(root,info);
    }catch(error){
      root.setAttribute(STATE_ATTRIBUTE,error&&error.code==="restricted"?"restricted":"failed");
    }
  }
  ...
  function scan(){
    queued=false;
    for(const root of document.querySelectorAll(ROOT_SELECTOR))processRoot(root);
  }
  function schedule(){
    if(queued)return;
    queued=true;
